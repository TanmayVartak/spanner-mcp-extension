...
allowed-tools:
  - mcp__plugin_spanner-plugin_spanner__get_database_ddl
---

Ask the user for the GCP project ID, instance ID, and database name if not provided in their message.

Fetch the schema using `mcp__plugin_spanner-plugin_spanner__get_database_ddl` with database set to `projects/<project-id>/instances/<instance-id>/databases/<database>`.
